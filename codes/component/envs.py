...
register(
    id='DeltaHedging-v0',
    entry_point='Envs:DeltaHedgingEnv',
)

register(
    id='DeltaHedgingTiming-v0',
    entry_point='Envs:DeltaHedgingEnvTiming',
)

register(
    id='DeltaHedgingMulti-v0',
    entry_point='Envs:DeltaHedgingEnvMulti',
)

register(
    id='DeltaHedgingFromdataset-v0',
    entry_point='Envs:DeltaHedgingEnvFromdataset',
)


# adapted from https://github.com/ikostrikov/pytorch-a2c-ppo-acktr/blob/master/envs.py
def make_env(env_id, seed, rank, episode_life=True, config=None):
    def _thunk():
        random_seed(seed)
...
class Task:
    def __init__(self,
        ...
        if log_dir is not None:
            mkdir(log_dir)
        envs = [make_env(name, seed, i, episode_life, config=config) for i in range(num_envs)]
        if single_process:
            Wrapper = DummyVecEnv
        else:
